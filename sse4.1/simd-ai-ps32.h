...
char sse4_1_ai_ps32_instructions[ sse4_1_ai_ps32_cnt + 1 ][ 100 ] = {
	"SSE4.1 32-bit Single-Precision Arithmetic Instructions",
	"dpps   \t_mm_dp_ps()         "
};

void* sse4_1_ai_ps32_bm_thread( void *arg ) {
	thread_data_t *td = (thread_data_t*)arg;
	// printf( "sse4_1_ai_ps32_bm_thread%u started\n", td->tid );

	uint64_t i;
	// uint32_t cx = 0;

	char name[ 25 ];
	sprintf( name, "sse4.1aips32th%u", td->tid );
	prctl( PR_SET_NAME, name );

	float ALIGN16 si[ 4 ] = { 8, 7, 6, 5 };
	float ALIGN16 sa[ 4 ] = { 1, 2, 3, 4 };
	...
	while ( td->thread_active ) {
		...
		// printf( "sse4_1_ai_ps32_bm_thread%u finish cycle #%u\n", td->tid, ++cx );
		pthread_mutex_unlock( &lock );

	}

	// printf( "sse4_1_ai_ps32_bm_thread%u stopped\n", td->tid );
	return NULL;
}

inline void sse4_1_ai_ps32_bm_threads_init( int32_t th_cnt ) {
	...
	// create threads
	for ( i = 0; i < threads_count; i++ ) {
		result = pthread_create( &td[i].th, &attr, sse4_1_ai_ps32_bm_thread, &td[i] );
...
inline void sse4_1_ai_ps32_bm_threads_start() {
	uint32_t i, c;

	// starting current threaded benchmark
	for ( c = 1; c <= sse4_1_ai_ps32_cnt; c++ ) {
		pthread_mutex_lock( &lock );
		for ( i = 0; i < threads_count; i++ ) {
			td[i].instruction = c;
		...
		pthread_mutex_unlock( &lock );
		print_results( sse4_1_ai_ps32_instructions[ c ], 8, cycles_count*threads_count, total_time );
	}

	return;
}

inline void sse4_1_ai_ps32_bm_threads_finit() {
	uint32_t i;
...
inline void sse4_1_ai_ps32_st_bm() {
	sse4_1_ai_ps32_bm_threads_init( 1 );
	sse4_1_ai_ps32_bm_threads_start();
	sse4_1_ai_ps32_bm_threads_finit();
	return;
}

inline void sse4_1_ai_ps32_mt_bm( int32_t th_cnt ) {
	sse4_1_ai_ps32_bm_threads_init( th_cnt );
	sse4_1_ai_ps32_bm_threads_start();
	sse4_1_ai_ps32_bm_threads_finit();
	return;
}
